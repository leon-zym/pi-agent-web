...
function makeStub(): StubSupervisor & Record<string, unknown> {
  ...
  (stub as unknown as Record<string, unknown>).cancelDialogsForSession = (workspaceId: string, sessionId: string) => {
    stub.cancelled.push({ workspaceId, sessionId });
    return 1;
  };
  (stub as unknown as Record<string, unknown>).sendExtensionUiResponse = (workspaceId: string, response: unknown) => {
