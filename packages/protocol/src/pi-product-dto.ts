...
export interface PiTextContentDto {
	type: "text";
	text: string;
...
export interface PiToolCallContentDto {
	type: "toolCall";
	id: string;
	name: string;
	arguments: unknown;
...
export interface UserMessageDto {
	role: "user";
	content: string | (PiTextContentDto | SessionImageContentDto)[];
	timestamp: number;
}

export interface PiAssistantMessageDto {
	role: "assistant";
	content: (PiTextContentDto | ThinkingContentDto | PiToolCallContentDto)[];
	usage: UsageDto;
...
export interface PiToolResultMessageDto {
	role: "toolResult";
	toolCallId: string;
	toolName: string;
	content: (PiTextContentDto | SessionImageContentDto)[];
	details?: unknown;
	usage?: UsageDto;
...
export interface PiBashExecutionMessageDto {
	role: "bashExecution";
	command: string;
	output: string;
	exitCode?: number;
...
export interface PiCustomMessageDto {
	role: "custom";
	customType: string;
	content: string | (PiTextContentDto | SessionImageContentDto)[];
	display: boolean;
...
export type PiSessionMessageDto =
	| UserMessageDto
...
export type PiSessionEntryDto =
	| (SessionEntryBaseDto & { type: "message"; message: PiSessionMessageDto })
...
export interface PiSessionTreeNodeDto {
	entry: PiSessionEntryDto;
...
export interface PiSessionCommandDataMap {
	prompt: undefined;
	steer: undefined;
	follow_up: undefined;
	abort: undefined;
	...
	get_entries: { entries: PiSessionEntryDto[]; leafId: string | null };
	get_tree: { tree: PiSessionTreeNodeDto[]; leafId: string | null };
	get_last_assistant_text: { text: string | null };
	set_session_name: undefined;
	get_messages: { messages: PiSessionMessageDto[] };
...
type SuccessResponseDto<K extends SessionCommandTypeDto> = {
	...
	command: K;
	success: true;
} & (PiSessionCommandDataMap[K] extends undefined
	? { data?: undefined }
	: { data: PiSessionCommandDataMap[K] });

export type PiSessionCommandResponseDto =
	| {
			id?: string;
			type: "response";
			command: string;
...
export type PiExtensionUiRequestDto =
	| {
...
export type PiAssistantMessageStreamEventDto =
	| { type: "start" }
	...
	| { type: "toolcall_end"; contentIndex: number; toolCall: PiToolCallContentDto }
	| { type: "done"; reason: "stop" | "length" | "toolUse" | "deferred"; message?: PiAssistantMessageDto }
	| { type: "error"; reason: "aborted" | "error"; error?: PiAssistantMessageDto };

export type PiSessionEventDto =
	| { type: "agent_start" | "turn_start" | "agent_settled" | "summarization_retry_finished" }
	| { type: "agent_end"; messages: PiSessionMessageDto[]; willRetry: boolean }
	| { type: "turn_end"; message: PiSessionMessageDto; toolResults: PiToolResultMessageDto[] }
	| { type: "message_start"; message: PiSessionMessageDto }
	| { type: "message_end"; message: PiSessionMessageDto }
	| { type: "message_update"; usage: UsageDto; assistantMessageEvent: PiAssistantMessageStreamEventDto }
	...
	| { type: "entry_appended"; entry: PiSessionEntryDto }
...
export type PiProductSessionEventDto = PiSessionEventDto | ExtensionErrorEventDto;
