...
### 面板可见性与恢复入口
- Details 面板的标题栏必须有明确的收起按钮；收起后保留一个固定的窄侧入口，点击它重新打开面板，不能只依赖历史状态或隐藏的快捷键。
- 从对话工具行打开 Inspector、从会话头部打开分支树，都会在面板已收起时自动恢复面板，并切到对应模式。
- 桌面端 Sidebar 在标题栏提供收起按钮；收起后变为 56px Rail，Rail 中提供展开按钮。窄屏自动进入 Rail，但不应让用户误以为工作区或会话被关闭。
- 所有收起/展开按钮都使用 aria-label 和 Tooltip，状态变化不能只通过宽度或颜色表达。

### Workspace 选择
- “添加 Workspace”触发运行 Gateway 所在操作系统的原生目录选择器，UI 只展示返回的绝对路径和确认按钮，不提供要求用户手动输入绝对地址的文本框。
- 取消选择是无副作用操作；系统没有可用的原生选择器时显示可解释的错误，不回退到静默猜测路径。
...
- 收起后的面板必须保留可发现、可键盘聚焦的重新打开入口；不得出现“收起后没有入口”的死路。
