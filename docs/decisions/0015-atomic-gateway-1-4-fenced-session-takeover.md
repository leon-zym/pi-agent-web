...
- Status: Accepted
- Date: 2026-08-31
- Amends: Browser/Gateway compatibility and control wording in ADRs 0003, 0007, 0010, 0011, and 0013

## Context

An observer can keep reading a Session while the controller's WebSocket remains connected, but could
not recover control when that controller page became unreachable: claim covers only a free lease,
while takeover changes both a strict Browser frame and the recipient-specific lease view. Retaining
a 1.3 success path would let peers disagree about fencing, lease revisions, and token redaction.

## Decision

1. Gateway protocol 1.4 is the single exact Browser/Gateway production contract. Protocol 1.3 and
   earlier versions are terminal mismatch fixtures only; they select no second DTO family,
...
2. Both hello directions require `session.fenced_takeover` alongside the existing production
   capabilities. A missing capability, an incomplete exact version, or an inconsistent hello
   terminates before Session traffic.
3. A `session_takeover` request identifies one canonical Session and carries its exact generation
   ...
   disconnect cleanup, and rekey through one lease transition domain, and performs a successful
   takeover as one compare-and-swap that replaces the owner and fencing token without stopping,
   restarting, cancelling, or migrating Pi work already admitted by the Runtime.
...
5. Catch-up and rekey preserve authoritative ordering: the Bridge keeps only the latest pending
   lease view per recipient and Session until its baseline is authoritative, and rekey publishes
...
command admitted before that point can complete to its original requester. Lease state stays
...
surfaces consume the revisioned control state separately; this decision creates no Workspace-wide,
multi-writer, remote, or persisted-control mode.
...
- **Broadcast one token-bearing status then redact it**: a shared payload can be retained or sent
  before redaction, so each recipient receives an independently constructed view.
