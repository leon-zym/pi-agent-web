...
ADR 0010's raster store binds one manifest media type to bytes identified by SHA-256, which works for an
admitted raster representation because its media type is verified against the bytes. Generic UTF-8 content
has no such identity: the same bytes may be valid text and valid JSON, and the carrying field decides
interpretation. Large tool arguments, partial results, results, message details, and Extension payloads would
otherwise be copied through decoding, projection, replay, snapshots, and WebSocket delivery, and replacing
reference-shaped objects recursively would let upstream data claim Gateway authority. The raster blob ceiling
is too small for some settled history, and chunking would add a second ordering protocol to every hold and
recovery path.
...
2. A `utf8` manifest records storage facts only and binds no semantic media type; raster metadata stays in
   the raster namespace. Both namespaces share the same store lifecycle lock, reservations, cache ledger,
   GC, holds, pins, and shutdown fence. The internal `utf8` namespace participates in store lookup and path
   derivation but is not exposed in the public reference.
3. The protocol uses one base UTF-8 content reference and three Gateway-owned wrappers:
   ...
   normalizes to `SessionInlineJsonDto` or `SessionExternalJsonDto`. No externalizable slot carries a bare
   JSON root, and the containing field reruns its guard after materialization; media type is a typed-slot
   concern. Content mode came from negotiated authority, never payload-shape inspection.
4. External JSON identity covers the exact UTF-8 bytes emitted for that slot; canonicalization is never
   applied. Both `inline_json.value` and the value encoded for `external_json` must pass the same bounded
   JSON guard. Two different serializations of equivalent JSON may have different digests without changing
   product semantics.
5. The externalizer uses a closed root-slot allowlist. Text slots are `TextContentDto.text` in reviewed
   tool-result and custom message or entry paths, `BashExecutionMessage.output`, Extension `editor.prefill`,
   and `set_editor_text.text`. JSON slots are `ToolCallContentDto.arguments`, tool execution `args`,
   `partialResult`, `result`, tool-result or custom message `details`, custom-message entry `details`, and
   the whole Extension `setWidget.widgetLines` array. The raw guard runs first, so a raw root resembling
   `inline_json`, `external_json`, or `content_ref` stays ordinary Pi data, and objects with those shapes
   below it are not traversed.
6. Everything else stays inline: nested JSON, assistant diagnostics, deferred and custom entry data,
   ...
   status text. Browser commands and Extension responses stay inline; no generic binary type, remote
   hosting, durable Session storage, or per-connection inline fallback.
7. A value whose encoded byte length stays strictly below the generic-content threshold keeps its string
   or `inline_json` form; one at or above the threshold uses `external_text` or `external_json` up to the
   blob ceiling, while raster images keep their own ceiling. Wrappers for the same bytes share one item and
   one hold. `packages/protocol/src/payload-budget.ts` owns the thresholds; a value beyond its ceiling
   fails admission instead of splitting.
8. Logical and wire bytes have separate accounting: a closed-slot walker counts inline text by UTF-8 byte
   ...
   including repeated roots that share one physical hold. A nested wrapper or reference lookalike remains
   ordinary JSON and contributes only through its containing inline JSON root. The walker does not recurse
   into opaque fields. The active-turn logical ceiling exceeds one admissible root and stays bounded.
...
12. Correlated response failures stay local only for evidenced content-blob or shared-cache ceilings and a
    ...
    unsafe manifest or path state, rollback failure, uncertain ownership, and every authoritative event or
    Extension externalization failure terminate the Runtime. A missing or stale content GET never yields an
    empty string, `null`, or empty object.
...
admission. Materialization moves some work from WebSocket parsing to authenticated GET and the final
consumer. JSON consumers may temporarily hold encoded text and the parsed value within the 48 MiB ceiling.
