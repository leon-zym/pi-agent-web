...
The Browser inserted discovered `@path` values into prompt text. Pi CLI can preprocess `@file`, but
RPC mode accepts only prompt text and inline images, so the interaction neither included content
reliably nor owned the security boundary it implied.

A path alone cannot show context cost or content risk. Splitting Host preview from a later path read
would also leave symlink and replacement races between the user's decision and capture.
